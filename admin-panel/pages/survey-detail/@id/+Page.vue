      ...
      <div>
        ...
        <div v-if="survey.status === SurveyStatus.DRAFT" class="text-right">
          <Panel class="text-sm w-60 text-center">
            <template #header><p class="text-center text-white">Warning!</p></template>
            <p class="text-left mb-5">
              Activating this survey will de-activate any existing surveys, and prevent it from being edited.
            </p>
            <button
    ...
    <Panel v-if="survey.status === SurveyStatus.ACTIVE || survey.status === SurveyStatus.ARCHIVED" class="mb-10 w-1/2">
      ...
      <div v-if="beacons.length">
        <template v-for="beacon in beacons" :key="beacon.beaconId">
          <div>
            <p>ID: {{ beacon.beaconId }}</p>
            <p>Name: {{ beacon.beaconName }}</p>
            <p>Device Key: {{ beacon.deviceKey }}</p>
          </div>
        </template>
      </div>
...
async function archiveSurvey(surveyId: number) {
