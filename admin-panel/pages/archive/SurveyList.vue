...
<template>
  <div v-if="surveys.length" class="bg-white relative shadow-md sm:rounded-lg overflow-hidden overflow-x-auto">
    <div class="grid grid-cols-[0.5fr_2fr_1fr_1fr_1fr_1fr_1fr_3fr]">
      ...
      <div class="grid-header">Beacons</div>
      <div class="grid-header">Readings</div>
      ...
      <template v-for="survey in surveys" :key="survey.id">
        ...
        <div class="grid-row">
          <span class="border text-xs font-medium px-1.5 py-0.5 rounded">{{ survey.beaconCount }}</span>
        </div>
        <div class="grid-row">
          <span class="border text-xs font-medium px-1.5 py-0.5 rounded">{{ survey.readingCount }}</span>
        </div>
...
defineProps<{
  surveys: SurveyWithBeaconReadingCounts[];
}>();
